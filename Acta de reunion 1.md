...
* Se ha puesto en marcha la aplicación CentroEducativo en el puerto 9090 de la máquina de portal
* Se ha configurado el proyecto web dinámico (`no12526`) en Eclipse IDE y se ha enlazado con Apache Tomcat v10.1
* Hemos redactado y probado el script de shell (`poblar.sh`) haciendo uso de `curl` para poblar la base de datos cada vez que se reinicia el servidor

## 3. Decisiones Técnicas y Librerías
* **Librerías HTTP:** Tras evaluar las opciones del enunciado, el equipo ha decidido utilizar **Apache HttpComponents (HttpClient 5.4)** para realizar las peticiones REST desde los servlets hacia CentroEducativo
* **Decodificación JSON:** Se utilizará la librería **Gson** (de Google) por su facilidad de uso para parsear las respuestas de la API
* **Filtro Logs:** Se ha implementado la Versión 2 del filtro (`Logs.java`), configurado exclusivamente a través del archivo `web.xml` sin usar anotaciones

## 4. Dificultades y Desacuerdos
* **Dificultad:** Eclipse IDE marcaba errores de sintaxis en el archivo `web.xml`. Se solucionó habilitando la resolución de entidades externas en las preferencias de XML de Eclipse
